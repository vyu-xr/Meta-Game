...
- Title: Grandma's Veil. Genre: Survival & Resource Management.
- Single-player, portrait, touch joystick with directional slash and human-form jump.
- Shared Body Connection health for the player soul and ritual; coins earned from night combat.
- One opening wave, two waves on later nights; retry forfeits half of that attempt's earnings only.
- Daytime travel connects home, shop, and circus. The free Human Form Card opens circus travel; upgrades remain locked.
- Circus: avoid bars, collect four parts, one collision ends the attempt, at most two attempts per day.
- Full three-stone acquisition and final reconnection are future scope, not complete in this prototype.
- AI-assisted planning, coding, iteration, and image generation using Codex and its image-generation tool; creator-supplied scene and texture assets.
- Three.js/HTML5, local vendor libraries, readable game logic in index.html, no external runtime requests.
...
## Session 62
- Used Codex to audit the live Devpost rules, FAQ, official design-intent template, and build-log guidance before packaging.
- Created a clean game.zip with index.html at the root, readable own game code inside it, local vendor libraries and license notices, and only referenced runtime assets.
- Assembled the offline asset data into the submission HTML, excluding the external generated data script and development tools from the ZIP.
- Created an anonymous, text-only 433-word design document using all seven exact template headings. Kept the old illustrated design draft unchanged outside the submission folder.
- Updated the locked decisions section and copied this running log as buildlog.md for upload.
- Extracted and tested the actual 19,556,721-byte ZIP with networking disabled on portrait and desktop viewports, plus a local-server portrait check. All five scenes rendered without WebGL errors, missing assets, or external requests.
- Ran state-driven checks of night failure/retry, night-clear instructions and Day 1, the Human Form Card and catalogue, and circus success. These checks do not replace a full manual balancing playthrough.
- Checked document word count, exact section headings, and absence of tables/images. Entrant eligibility and creator-supplied asset/audio rights still require the creator's confirmation.

## Session 63
- Kept the opening logo silent and moved audible music playback to the start of the story, immediately after the logo fades out.
- A tap during the logo unlocks muted audio; the track restarts from its beginning at 25% volume when the logo ends. If the browser blocks automatic playback, a later tap still starts it.
- Rebuilt the submission ZIP for this audio-timing update.
